...
char2dna(list(seq1 = "TAACGC", seq2 = "ATTGCG")) -> newwoodmouse
dna2char(newwoodmouse) -> wood_chars
mutated_bin  <- mutation(newwoodmouse, format = "bin", n.mutations = 1)
...
test_that("output length matches input length with many seqs in input", {
  ...
  dna2char(newwoodmouse) |> enframe() -> comparable_woodmouse
...
test_that("mutated sequences differ from original", {
  ...
  differences <- mapply(function(orig, mut) any(orig != mut), newwoodmouse, mutated_bin)
...
test_that("error triggers with invalid arguments", {
  expect_error(mutation(newwoodmouse, format = "bin", n.mutations = NA, prob.mutation = NA))
  expect_error(mutation(newwoodmouse, format = "bin", n.mutations = 2, prob.mutation = 0.1))
